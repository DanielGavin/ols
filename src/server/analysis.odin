...
resolve_selector_expression :: proc(ast_context: ^AstContext, node: ^ast.Selector_Expr) -> (Symbol, bool) {
	...
	if ok := internal_resolve_type_expression(ast_context, node.expr, &selector); ok {
		...
		case SymbolPackageValue:
			...
			if node.field != nil {
				//log.infof("Looking up field '%v' in package '%v'", node.field.name, selector.pkg)
				field_symbol, ok := lookup(node.field.name, selector.pkg, node.pos.file)
				if ok {
					//log.infof("Found field_symbol with type %v", field_symbol.type)
					// Check if the field is a package alias (constant/variable pointing to a package)
					#partial switch v in field_symbol.value {
					case SymbolBasicValue:
						if v.ident != nil {
							//log.infof("SymbolBasicValue: ident.name = '%v'", v.ident.name)
							...
							if strings.contains(v.ident.name, "/") {
								//log.infof("Treating as package: %v", v.ident.name)
								pkg_symbol := Symbol {
									type = .Package,
									pkg = v.ident.name,
					...
					case SymbolGenericValue:
						if v.expr != nil {
							if ident, ident_ok := v.expr.derived.(^ast.Ident); ident_ok {
								//log.infof("SymbolGenericValue: ident.name = '%v'", ident.name)
						// Check if the identifier name contains "/" (indicating it's been replaced with a full package path)
						if strings.contains(ident.name, "/") {
							//log.infof("Treating as package: %v", ident.name)
							pkg_symbol := Symbol {
								type = .Package,
								pkg = ident.name,
								value = SymbolPackageValue{},
							}
							// Also try to look up symbols in the package to verify it exists
							//if test_symbol, test_ok := lookup("", ident.name, ""); !test_ok {
							//	log.infof("Warning: package '%v' appears to be empty or doesn't exist in index", ident.name)
							//}
							return pkg_symbol, true
						}
								// Try to resolve the identifier to see if it's a package
