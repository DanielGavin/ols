...
if "%1" == "test" (
    odin test tests -collection:src=src -define:ODIN_TEST_FAIL_ON_BAD_MEMORY=true -extra-linker-flags:"/STACK:4000000,2000000"
) else if "%1" == "single_test" (
    odin test tests -collection:src=src -define:ODIN_TEST_NAMES=%2 -define:ODIN_TEST_FAIL_ON_BAD_MEMORY=true -extra-linker-flags:"/STACK:4000000,2000000"
) else if "%1" == "build_test" (
	odin build tests -build-mode:test -collection:src=src -define:ODIN_TEST_FAIL_ON_BAD_MEMORY=true -extra-linker-flags:"/STACK:4000000,2000000" %2
) else if "%1" == "debug" (
